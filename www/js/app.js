...
var app = {
    ...
    onDeviceReady: function() {
        ...
        if( /(android)/i.test(navigator.userAgent) ) { 
            admobid = { // for Android
                banner: 'ca-app-pub-9588717711097708/1137818873',
                interstitial: 'ca-app-pub-6869992474017983/1657046752'
            };
        } else if(/(ipod|iphone|ipad)/i.test(navigator.userAgent)) {
            admobid = { // for iOS
                banner: 'ca-app-pub-9588717711097708/4091285279',
   ...
   // RSS: Feeds Controller
    app.controller('FeedsController', function($scope, $http, FeedData, FeedStorage) {
        ...
        var getData = function (keyword) {
            ...
            alert(keyword);
            $http({method: 'JSONP', url: 'http://ajax.googleapis.com/ajax/services/feed/load?v=1.0&num=50&callback=JSON_CALLBACK&q=' + encodeURIComponent(FeedData.url+'/?s='+keyword)}).
            success(function(data, status, headers, config) {
        ...
        $scope.getData1 = function(){
            ...
            $http({method: 'JSONP', url: 'http://ajax.googleapis.com/ajax/services/feed/load?v=1.0&num=50&callback=JSON_CALLBACK&q=' + encodeURIComponent(FeedData.url+'/?s='+$scope.search.toLowerCase())}).
            success(function(data, status, headers, config) {
        ...
        $scope.removeBookmark = function (title){
